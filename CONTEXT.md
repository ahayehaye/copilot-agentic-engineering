...
| **Implementer** | A workflow role that guides the user-run implementation phase (Phase 2): the user runs `/implement`, `/verify-ac`, and `/code-review` in their own session, and the implementer names the command with the correct arguments, checks preconditions, tracks progress from the tracker and git, and interprets halts and findings. A guide, not an executor or a watcher — no sub-agents, no journal; the tracker and git are the only state, so a cleared session costs nothing — it reconstructs where things stand from those alone. Editing is bounded to two categories: **skill-bound** edits a skill's workflow owns (the user runs the skill; the implementer never makes one) and **instruction-bound** edits the user explicitly instructs — no other edit. The self-driven alternative to the director for small changes; when the open work is more than a handful of slices it notes the director may be the better tool — a note, not a gate. A **Copilot Agent** under the `copilot-agents/` **Source**, deployed by the **Agent Manager** and launchable via the **Copilot Launcher**. The verified state is keyed on checked AC boxes — the checked box is the durable, machine-detectable marker, the per-AC comments a loose human-readable record. The *why* is recorded in [ADR-0016](docs/adr/0016-implementer-is-a-guide-in-the-users-session.md) and [ADR-0017](docs/adr/0017-self-run-flow-verifies-ac-inline.md). |
