...
| **External Skill** | A skill referenced by this repo's agents or docs but not shipped under `skills/`. It is assumed to be already installed in any environment that uses this repo (e.g. the Matt Pocock skill set: `code-review`, `grilling`, `to-spec`, `to-tickets`, `grill-with-docs`, `domain-modeling`). The repo documents the dependency; it never ships or re-invents the skill. |
