...
> [!NOTE]
> **Scope and the word "agent".** This page documents the **Copilot Agent** *item type* — the installable `*.agent.md` files managed by the Agent Manager. It does **not** describe the pipeline *roles* (analyst, director, implementer, worker, reviewer, axis-reviewer — plus scout, a utility role outside the workflow) that run the workflow skills. Keep the two senses of "agent" separate:
>
> - **Agent (item type):** an installable `*.agent.md` under `copilot-agents/`, deployed to `~/.copilot/agents/`. This page is about these.
> - **Agent (pipeline role):** the participant role — analyst, director, implementer, worker, reviewer, or axis-reviewer — that plays out in the workflow, plus scout, a utility role outside the workflow. This page does not describe what they do.
