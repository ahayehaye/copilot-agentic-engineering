...
### Gate 2: Flavor — GitHub and local markdown
...
If the flavor is **GitHub**, continue — the seed for the GitHub flavor is the template files `issue-tracker-github.md` and `agents-md-github.md` in this skill's directory (see [Seed: the GitHub enhancement](#seed-the-github-enhancement) below).

If the flavor is **local markdown**, continue — the seed for the local-markdown flavor is the template files `issue-tracker-local-markdown.md` and `agents-md-local-markdown.md` in this skill's directory (see [Seed: the local-markdown enhancement](#seed-the-local-markdown-enhancement) below). The label-creation step does not apply to this flavor (labels are recorded as a `Labels:` line in the ticket body, per the tracker doc); the label-rule block and the tracker-doc sections do.

Otherwise, stop and say:

> `/setup-alex-skills` supports the GitHub and local-markdown flavors. This repo's tracker is <flavor>. Azure DevOps support is tracked upstream; GitLab is a future flavor.
...
- Check the tracker's label state (GitHub flavor only): `gh label list --json name,description` — do `spec` and `vertical-slice` exist, and with what descriptions? (Local-markdown flavor: labels are a `Labels:` line in the ticket body — no label creation step.)

### 2. Present findings and ask

Summarize what's present and what's missing — one line per enhancement (including the label-rule block in the target file) and, for the GitHub flavor, one line per standard label (exists / missing / description mismatch).

If every enhancement is present and unmodified (and, for the GitHub flavor, both labels exist with the canonical descriptions), the repo is already enhanced: report that the re-run is a no-op and stop.

Otherwise, build the delta: diff the existing doc against the enhanced form (the flavor's tracker template — `issue-tracker-github.md` or `issue-tracker-local-markdown.md` — adapted to this repo), and diff the target file against the form with the label-rule block in place (the flavor's agents-md template). The delta is additive only — the upstream sections (conventions, PR-surface flag, wayfinding, everything the upstream setup wrote) are preserved untouched. The user's hand edits to existing sections are preserved: the delta fills only the gaps — missing sections are added, drifted sections (and a drifted label-rule block) are presented side by side so the user decides, and nothing is silently clobbered.
...
- The label-rule block for the target file (`AGENTS.md` or `CLAUDE.md`) — the block from the flavor's agents-md template ([agents-md-github.md](./agents-md-github.md) or [agents-md-local-markdown.md](./agents-md-local-markdown.md)), shown in its placement: inside the `### Issue tracker` subsection of the `## Agent skills` block (the subsection is added if missing)
- The label actions (GitHub flavor only): create each missing label with its canonical description; leave existing labels as-is, flagging any description mismatch
...
- Edit the target file idempotently: if the marked block (`<!-- setup-alex-skills:label-rule -->` … `<!-- /setup-alex-skills:label-rule -->`) is present, replace it in place with the block from the flavor's agents-md template; if absent, insert it inside the `### Issue tracker` subsection of the `## Agent skills` block, adding the subsection when missing. Touch nothing else in the file.
...
## Seed: the local-markdown enhancement

The same three sections, in the local-markdown flavor's own concrete commands, are the local-markdown template, [issue-tracker-local-markdown.md](./issue-tracker-local-markdown.md), in this skill's directory — the single source for the tracker-doc enhancement content for that flavor. The label-rule block written into the target file is the template [agents-md-local-markdown.md](./agents-md-local-markdown.md), beside it. The label-creation step does not apply to this flavor (labels are a `Labels:` line in the ticket body). The reference instance for this flavor is the ledger skeleton's tracker doc — a deployed ledger workspace is already enhanced, so a re-run there is a no-op; the flavor's value is reconciliation of pre-existing local-markdown workspaces and drift after template changes.
